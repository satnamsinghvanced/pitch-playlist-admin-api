...
const generateCuratorListYear = async (startDate, endDate) => {
  try {
    const result = await topCuratorAdminListYear.deleteMany({});
    console.log(`✅ Deleted ${result.deletedCount} documents from topCuratorAdminListYear.`);
    ...
    await Promise.all(topUsersWithPosition.map(val => new topCuratorAdminListYear(val).save()));
...
export default generateCuratorListYear;
