...
const generateCuratorListSevenDays = async (startDate, endDate) => {
  try {
    const result = await topCuratorAdminListSevenDays.deleteMany({});
    console.log(`✅ Deleted ${result.deletedCount} documents from topCuratorAdminListSevenDays.`);
    ...
    await Promise.all(topUsersWithPosition.map(val => new topCuratorAdminListSevenDays(val).save()));
...
export default generateCuratorListSevenDays;
